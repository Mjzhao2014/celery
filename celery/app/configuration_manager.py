...
class ConfigurationManager:
    ...
    def config_from_object(self, obj, silent: bool = False):
        try:
            if isinstance(obj, str):
                loaded = self.loader.config_from_object(obj, silent=silent)
                if not loaded:
                    if silent:
                        return None
    ...
    def read_configuration(self, env: str = 'CELERY_CONFIG_MODULE') -> Dict[str, Any]:
        ...
        self.config_from_object(module_name, silent=False)
        try:
            return dict(self.app.conf)
        except TypeError as exc:
            raise ImproperlyConfigured(
                f"Configuration loaded from {module_name!r} is not dict-like"
            ) from exc
