...
def test_register_task_raises_on_duplicate():
    ...
    assert manager.tasks["mytask"] is first
...
def test_signal_manager_connect_method_direct():
    ...
    assert {name for name, _ in call_log} == {"handler1", "handler2"}
    for _, payload in call_log:
        assert payload.get("test_data") == "connect_test"
